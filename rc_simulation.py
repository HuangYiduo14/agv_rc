...
v_fleet_traj = {ind: network1.deterministic_shortest_path(od[0][0],od[0][1],time0=od[1],has_time=True) for ind, od in virtual_fleet.items()}
...
possible_od = [('half shelf','workstation'),('full shelf','workstation'),('workstation','half shelf'),('workstation','full shelf'),('workstation', 'workstation')]
...
for t in range(100,100 + n_t):
    ...
    for od in possible_od:
        ...
        for i in range(from_node.shape[0]):
            ...
            agv_index+=1
            trip_v_agv, travel_time, delay, traj_table = new_agv.rc_shortest_path_routing(network1, traj_table)
            total_travel_time += travel_time
            total_delay += delay
            travel_time_record.append(travel_time)
            delay_record.append(delay)
            trip_v_agv_record.append(trip_v_agv)
plt.figure()
